    ...
    fn started(&mut self, ctx: &mut Self::Context) {
        ...
        ctx.run_interval(Duration::from_millis(110), move |act, ctx| {
            ...
            if act.elapses_left == 0 {
                act.sender
                    .try_send(act.instant.take().expect("To have Instant"))
...
async fn test_run_interval() {
    ...
    let result = actix_rt::time::timeout(MAX_WAIT, receiver.recv())
        .await
