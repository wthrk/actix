...
use actix::prelude::*;
use actix_rt::time::{interval_at, sleep, Instant, Interval};
...
#[wasm_bindgen_test]
async fn test_add_timeout_cancel() {
    System::new()
        .block_on(async {
...
async fn test_add_timeout_stop() {
    System::new()
        .block_on(async {
            let _addr = MyActor {
                op: Op::TimeoutStop,
...
#[wasm_bindgen_test]
async fn test_message_stream_wait_context() {
    let m = Arc::new(AtomicUsize::new(0));
    let m2 = Arc::clone(&m);
...
#[wasm_bindgen_test]
async fn test_stream_wait_context() {
    let m = Arc::new(AtomicUsize::new(0));
    let m2 = Arc::clone(&m);
...
#[wasm_bindgen_test]
async fn test_stream_nowait_context() {
    let m = Arc::new(AtomicUsize::new(0));
    let m2 = Arc::clone(&m);
...
#[wasm_bindgen_test]
async fn test_current_context_handle() {
...
#[wasm_bindgen_test]
async fn test_start_from_context() {
    let h = Arc::new(AtomicUsize::new(0));
    let h2 = Arc::clone(&h);
    let m = Arc::new(AtomicUsize::new(0));
    let m2 = Arc::clone(&m);
...
#[wasm_bindgen_test]
async fn test_cancel_completed_with_no_context_item() {
    actix::System::new()
        .block_on(async {
...
mod scheduled_task_is_cancelled_properly {
    ...
    struct TestActor(Option<oneshot::Receiver<()>>);
    ...
    impl Actor for TestActor {
        ...
        fn started(&mut self, ctx: &mut Self::Context) {
            if let Some(rx) = self.0.take() {
                ...
                panic!("This actor must not be started more than once");
            }
        }
    }
    ...
    async fn cancels() {
        ...
        sys.block_on(async {
            ...
            let addr = TestActor(Some(rx2)).start();
...
mod restart {
    ...
    #[wasm_bindgen_test]
    async fn cancels_spawned_futures() {
